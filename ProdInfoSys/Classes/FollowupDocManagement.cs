    ...
    public class FollowupDocManagement
    {
        ...
        public FollowupMetadata GetDataForNestedReportViews()
        {
            ...
            if (_callerEntity == EnumCallerEntity.QRQC)
            {
                ...
                ret.KftDailyPlannedQty = new ObservableCollection<DailyPlan>(_doc.StatusReportsQRQC.Where(s => s.ReportName == _selectedReportName).SelectMany(s => s.DailyPlans));
            ...
            if (_callerEntity == EnumCallerEntity.PROD)
            {
                ...
                ret.KftDailyPlannedQty = new ObservableCollection<DailyPlan>(_doc.StatusReports.Where(s => s.ReportName == _selectedReportName).SelectMany(s => s.DailyPlans));
            }
